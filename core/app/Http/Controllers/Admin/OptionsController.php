...
class OptionsController extends Controller
{
    ...
    public function store(Request $request, $id = 0)
    {
        $request->validate([
            'name'       => 'required',
            'dividend'   => 'required|numeric|regex:/^\d+(\.\d{1,2})?$/',
            'divisor'    => 'required|numeric|regex:/^\d+(\.\d{1,2})?$/',
        ]);
    ...
    {
        $options        = Option::where('question_id', 1)->get();
        $pageTitle      = 'Manage Scores';
        $emptyMessage   = 'No option found';
        return view('admin.option.edit', compact('pageTitle', 'emptyMessage', 'options'));
    }
