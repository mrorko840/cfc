...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        return view('admin.users.detail', compact('pageTitle', 'user','totalDeposit','totalWithdraw','totalTransaction','totalBet','reff','totalReferral','totalCommission','countries'));
    }
    ...
    public function send_notification($id){
        ...
        return view('admin.users.notification', compact('pageTitle','user'));
    }
    
    public function send_notification_post($id,Request $request){
        $request->validate([
            'message' => 'required|string|max:65000',
            'subject' => 'required|string|max:190',
        ]);
         ...
         $notification->save();
        $notify[] = ['success', 'Notification sent to '.$user->username];
        return back()->withNotify($notify);
    }
    
    public function notification_log($id){
        $user = User::findOrFail($id);
        $pageTitle = 'Notification log of '.$user->username;
        $logs = Notification::where('user_id',$id)->with('user')->orderBy('id','desc')->paginate(getPaginate());
        $emptyMessage = 'No data found';
        return view('admin.users.notification_log', compact('pageTitle','logs','emptyMessage','user'));
    }
    ...
    public function notification_details($id){
        ...
        return view('admin.users.notification_details', compact('pageTitle','user', 'notification'));
    }
    
    public function notification_update($id,Request $request){
        $request->validate([
            'message' => 'required|string|max:65000',
            'subject' => 'required|string|max:190',
        ]);
